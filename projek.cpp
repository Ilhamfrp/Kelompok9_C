...
void hapusKontak(Kontak*& head, const string& keyword, bool blokir = false) {
    ...
    // Mencari kontak yang cocok dengan nama yang diberikan
    while (temp != nullptr) {
        if (toLower(temp->nama).find(lowerKeyword) != string::npos) {
            daftarCocok.push_back(temp);
        }
    ...
    if (daftarCocok.size() > 1) {
        ...
        // Pastikan untuk membersihkan buffer input setelah cin >> pilih
        cin >> pilih;
        cin.ignore(); // Membersihkan sisa karakter newline yang tersisa dari input
